...
public class Dijkstra {
	...
	private HashMap<NodeId, Double> distanceMatrix;//某一目标起点，到某点的距离
	private HashMap<NodePair, Double> shortestLength;//两点之间的距离
	private ArrayList<Edge> shortestPath; //最短路径的路段序列
	...
	public ArrayList<Edge> getShortestPath(NodeId sourceNid, NodeId sinkNid){
		...
		while(!parent.equals(sourceNid)){
			...
			for(Iterator<NodeId> it = neighborsFrom.iterator(); it.hasNext();){
				...
				if(!this.shortestLength.containsKey(spPair))
					continue;
	...
	private void recurseSearchNeighborSets(NodeId nid){
		if(neighborSets.containsKey(nid)){
			HashSet<NodeId> neighborSet = neighborSets.get(nid);//终点集
			for(Iterator<NodeId> it = neighborSet.iterator(); it.hasNext();){
				NodeId neighborId = it.next();//遍历终点集
	...
	private void initializeDistanceMatrix(){
		...
		for(int index = 1; index < sinkSet.size(); index++){
