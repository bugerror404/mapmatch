...
public class PFAnalysis {
	...
	private HashMap<String, Double> observationProbabilities;
	...
	public PFAnalysis(Map map, CandidatePreparation cp, double mu, double sigma, boolean runRule1, boolean runRule2, boolean runRule3){
		this.setMu(mu);
		this.setSigma(sigma);
	...
	private void setObservationProbabilities(CandidatePreparation cp, boolean runRule1){
		...
		for(Time time: candidates.keySet()){
			...
			for(int index = 0; index < candList.size(); index++){
				...
				double obsProb = 1;
				if(runRule1)
					obsProb = this.runObservationProbabilityFunction(candidate);
		...
		for(int index = 1; index < times.size(); index++){
			...
			for(int iNow = 0; iNow < candListNow.size(); iNow++){
				for(int iPre = 0; iPre < candListPre.size(); iPre++){
					...
					this.candidateGraph.put(candPair, spatialWeight);
				}
			}
		}
	}
	...
	private double runTransmissionProbabilityFunction(Dijkstra dijkstra, Map map, Point pNow, Point pPre, Candidate cNow, Candidate cPre){
		...
		for(Edge edgePre: cPre.edgeAlpha.keySet()){
			...
			for(Edge edgeNow: cNow.edgeAlpha.keySet()){
				...
				if(cNow.lon == cPre.lon && cNow.lat == cPre.lat){
					length = 0;
					path = null;
		...
		double tra = pointDistance / minLength;
		traProb = 1/(Math.sqrt(2*Math.PI)*this.sigma) * Math.pow(Math.E, -(Math.pow((1-tra), 2))/(2*Math.pow(this.sigma, 2)));
		return traProb;
	}
	...
	private double runTemporalFunction(Map map, Candidate cPre, Candidate cNow){
		...
		if(this.shortestPath.get(candPair) == null) return 0;
		if(this.shortestPath.get(candPair).isEmpty()) return 0;
		if(this.shortestLength.get(candPair) == 0) return 0;
		ArrayList<Edge> path = this.shortestPath.get(candPair);
		double length = this.shortestLength.get(candPair);
		double deltaT = Math.abs(cNow.time.epoch - cPre.time.epoch)/1000;
		double vMean = (length/deltaT) * 3.6;
		double sumEM = 0;
		double sumE2 = 0;
		double sumM2 = 0;
	...
	public HashMap<CandidatePair, ArrayList<Edge>> getShortestPath() {
		return shortestPath;
	}

	public HashMap<CandidatePair, Double> getShortestLength() {
